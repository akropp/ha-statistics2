    ...
    async_add_entities(
        new_entities=[
            StatisticsSensor(
                ...
                pin_oldest=config[CONF_PIN_OLDEST],
            )
        ],
...
class StatisticsSensor(SensorEntity):
    ...
    def __init__(
        ...
        self._pin_oldest: bool = pin_oldest
        ...
        self._pin_oldest_fn: Callable[[float | bool, datetime, datetime], tuple[float | bool, datetime]]
        if self.is_binary:
            self._state_characteristic_fn = getattr(
                self, f"_stat_binary_{self._state_characteristic}"
            )
            if self._pin_oldest:
                self._pin_oldest_fn = getattr(self, f"_pin_binary_{self._state_characteristic}", self._pin_default_fn)
        else:
            self._state_characteristic_fn = getattr(
                self, f"_stat_{self._state_characteristic}"
            )
            if self._pin_oldest:
                self._pin_oldest_fn = getattr(self, f"_pin_{self._state_characteristic}", self._pin_default_fn)

        self._update_listener: CALLBACK_TYPE | None = None
    
    def _pin_default_fn(self, lastval: float | bool, lastage: datetime, oldage: datetime) -> tuple[float | bool, datetime]:
        return None, None
    ...
    def _purge_old_states(self, max_age: timedelta) -> None:
        ...
        lastval: float | bool = None
        lastage: datetime = None
        ...
        while self.ages and (now - self.ages[0]) > max_age:
            ...
            lastage = self.ages.popleft()
            lastval = self.states.popleft()
    ...
    # pin functions for numeric sensor
    def _pin_average_linear(self, lastval: float | bool, lastage: datetime, oldage: datetime) -> tuple[float, datetime]:
        return lastval + (oldage - lastage).total_seconds() * (self.states[0] - lastval) / (self.ages[0] - lastage).total_seconds(), oldage

    def _pin_average_step(self, lastval: float | bool, lastage: datetime, oldage: datetime) -> tuple[float, datetime]:
        return lastval, oldage
    ...
    # pin functions for numeric sensor
    def _pin_binary_average_step(self, lastval: float | bool, lastage: datetime, oldage: datetime) -> tuple[float, datetime]:
        return lastval, oldage
